...
Router.route('/children', {
  ...
  waitOn: function() {
    ...
    return subs.subscribe('kids', user.profile.groupId[0]);
  }
});
...
Router.route('/expenses', {
  ...
  waitOn: function() {
    var user = Meteor.user();
    return [subs.subscribe('expenses', user.profile.groupId[0]), , subs.subscribe('groupUsers', user.profile.groupId[0])];
  }
});
Router.route('/managegroup', {
  ...
  waitOn: function() {
    var user = Meteor.user();
    return [subs.subscribe('groupMembers', user.profile.groupId[0]), subs.subscribe('groupUsers', user.profile.groupId[0])];
  }
});
Router.route('/newsfeed', {
  ...
  waitOn: function() {
    var user = Meteor.user();
    return [subs.subscribe('news', user.profile.groupId[0]), , subs.subscribe('groupUsers', user.profile.groupId[0])];
  }
});
